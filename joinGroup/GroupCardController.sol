...
import "../buycard/FireWorksCard.sol";

contract GroupCardController is Group, GroupOwnable {
    ...
    function joinCard(uint256 _systemType, uint256 _tokenId) public returns(bool) {
        
        Param memory param = groupDataController.getadmissionFee(_systemType);
        require(param.payTokenAddress != address(0), "GroupCardController: join method _no not exists");
        require(param.payType == 1, "GroupCardController: pay type error");
        require(param.isOpen, "-1001");
        
        accountController = AccountController(accountContractAddress);
        (address accountAddress, address uperAddressArr, bool isTeamLeader, bool isOpen) = accountController.getAccountInfo(accountSystemNoMapping[_systemType], msg.sender);
        ...
        FireWorksCard card = FireWorksCard(param.payTokenAddress);
        card.transferFrom(msg.sender, address(this), _tokenId);
        
        if(currentGroup.joinAddress.length == param.groupCountLimit) {
            // Is full
            groupDataController.changeGroupFull(_systemType, currentGroupId);
            if(groupDataController.getUserDraw()) {
                drawPrizeCard(currentGroupId, _systemType);
            }
        ...
        groupDataController.addGrouopJoinAndInviterCard(_systemType, currentGroupId, _tokenId);
    ...
    function drawPrizeCardManage(uint256 _joinId, uint256 _no) public onlyDrawPrizeOwner returns(bool) {
        return drawPrizeCard(_joinId, _no);
    }
    
    function drawPrizeCard(uint256 _joinId, uint256 _no) private returns(bool) {
        ...
        distributeCardJoinAndInviteReward(currentGroup, winerIndex, token, card, param, _no);
        
        // add miner
        minerController = MinerController(minerContractAddress);
        (bool success) = minerController.addMiner(accountSystemNoMapping[_no], currentGroup.winnerAddress);
        ...
        bool carveFeeSuccess = token.transferFrom(receiveAddressMapping[_no], carvesParam.carvesAccountAddress[0], param.admissionFee*param.groupCountLimit);
         ...
         for(uint256 i = 2; i < carvesParam.carvesAccountAddress.length; i++) {
            bool carveMarketSuccess = token.transferFrom(receiveAddressMapping[_no], carvesParam.carvesAccountAddress[i], 
                                            param.deposit
    ...
    function distributeCardJoinAndInviteReward(JoinGroup memory currentGroup, uint256 winerIndex, 
                    ERC20PresetMinterPauser token, FireWorksCard card, Param memory param,
                    uint256 _no) private {
        address[] memory groupAddress = currentGroup.joinAddress;
        uint256[] memory tokenIds = currentGroup.tokenIds;
        ...
        for(uint256 i = 0; i < groupAddress.length; i++) {
            ...
            if(groupAddress[i] != currentGroup.joinAddress[winerIndex]) {
                card.transferFrom(address(this), groupAddress[i], tokenIds[i]);
                
                bool success1 = token.transferFrom(receiveAddressMapping[_no], groupAddress[i], param.joinReward);
                require(success1, "GroupController: drawPrize method fail, distribute join reward fail");
                sendReward += param.joinReward;
                groupDataController.increaseAccountJoinIncome(groupAddress[i], _no, param.joinReward);
            }
            // inviter reward
            if(currentGroup.inviterAddress[i] != address(0)) {
                bool success2 = token.transferFrom(receiveAddressMapping[_no], currentGroup.inviterAddress[i], param.inviterReward);
                require(success2, "GroupController: drawPrize method fail, distribute inviter reward");
                sendReward += param.inviterReward;
    ...
    **/
    ...
    mapping(uint256 => address) public receiveAddressMapping; // systemNo => receive address
    function setReceiveAddress(uint256 _systemNo, address _receiveAddress) public onlyOwner {
        receiveAddressMapping[_systemNo] = _receiveAddress;
    }
    mapping(uint256 => uint256) public accountSystemNoMapping; // systemNo => account systemNo
    function setAccountSystem(uint256 _systemNo, uint256 _accountSystemNo) public onlyOwner {
        accountSystemNoMapping[_systemNo] = _accountSystemNo;
    }
    ...
    function ownerCardWithdraw(uint256 _no, uint256 _tokenId) public onlyOwner returns(bool) {
        Param memory param = groupDataController.getadmissionFee(_no);
        FireWorksCard card = FireWorksCard(param.payTokenAddress);
        card.transferFrom(address(this), owner, _tokenId);
    ...
    function carvesTenFun(uint256 _no) public onlyParamOwner returns(bool) {
        ...
        for(uint256 i = 0; i < carvesTenParam.tenuAddress.length; i++) {
            ...
            bool carveSuccess = token.transferFrom(receiveAddressMapping[_no], carvesAccount, transferAccountVaue);
